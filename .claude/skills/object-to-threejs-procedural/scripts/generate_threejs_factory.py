...
GENERATED_MARKER = "// @generated by threejs-object-sculptor; edit a wrapper file, not this file."
...
def _validate_generation_spec(
    ...
    proof = (
        ...
        if errors
            ...
            render_contract_sha256=render_pipeline_contract_sha256(spec),
...
def _validation_proof_matches(
    ...
    return (
        ...
        and proof.render_contract_sha256 == render_pipeline_contract_sha256(spec)
    )
...
def assessed_shading_treatment(spec: dict[str, Any]) -> str:
    ...
    return primary if isinstance(primary, str) and primary else "physically-based"
...
def typescript_review_pipeline_helpers(
    contract: dict[str, Any],
    contract_sha256: str,
) -> list[str]:
    source = r"""
export type SculptResolvedRenderMode = 'native-msaa' | 'smaa' | 'fxaa' | 'off';

export type SculptRenderPipelineContract = {
  version: 1;
  status: 'required' | 'not-required';
  backend: 'webgl';
  antiAliasing: {
    mode: 'auto' | 'native-msaa' | 'smaa' | 'fxaa' | 'off';
    qualityPreset: 'quality' | 'performance';
    fallbackOrder: Array<'smaa' | 'fxaa'>;
  };
  maxPixelRatio: number;
  fallbackPolicy: 'fail-closed';
};

export type SculptRenderReceipt = {
  artifactType: 'threejs-sculpt-render-receipt';
  version: 1;
  contractSha256: string;
  backend: 'webgl';
  requestedMode: SculptRenderPipelineContract['antiAliasing']['mode'];
  resolvedMode: SculptResolvedRenderMode;
  fallbackReason: string;
  passChain: string[];
  outputTransformOwner: 'renderer' | 'OutputPass';
  contextAntialias: boolean;
  defaultFramebufferSamples: number;
  maximumRenderTargetSamples: number;
  antialiasVerified: boolean;
  logicalWidth: number;
...
export type SculptReviewPipelineController = {
  render: (deltaTime?: number) => void;
  resize: (width: number, height: number, pixelRatio: number) => void;
  diagnostics: () => SculptRenderReceipt;
  receipt: () => SculptRenderReceipt;
  dispose: () => void;
};

export const SCULPT_REVIEW_RENDER_CONTRACT =
  __CONTRACT__ as SculptRenderPipelineContract;
export const SCULPT_REVIEW_RENDER_CONTRACT_SHA256 = __HASH__;

type SculptRenderPipelineHost = typeof globalThis & {
  __THREEJS_SCULPT_REVIEW_PIPELINES__?: Map<string, SculptReviewPipelineController>;
  __THREEJS_SCULPT_REVIEW_PIPELINE_RESERVATIONS__?: Set<string>;
};

function sculptRenderPipelineRegistry(): Map<string, SculptReviewPipelineController> {
  const host = globalThis as SculptRenderPipelineHost;
  const registry = host.__THREEJS_SCULPT_REVIEW_PIPELINES__ ?? new Map();
  host.__THREEJS_SCULPT_REVIEW_PIPELINES__ = registry;
  return registry;
}

function sculptRenderPipelineReservations(): Set<string> {
  const host = globalThis as SculptRenderPipelineHost;
  const reservations =
    host.__THREEJS_SCULPT_REVIEW_PIPELINE_RESERVATIONS__ ?? new Set();
  host.__THREEJS_SCULPT_REVIEW_PIPELINE_RESERVATIONS__ = reservations;
  return reservations;
}

function sculptStableJson(value: unknown): string {
  if (Array.isArray(value)) {
    return `[${value.map((item) => sculptStableJson(item)).join(',')}]`;
  }
  if (value !== null && typeof value === 'object') {
    const entries = Object.entries(value as Record<string, unknown>)
      .sort(([left], [right]) => left.localeCompare(right));
    return `{${entries.map(([key, item]) =>
      `${JSON.stringify(key)}:${sculptStableJson(item)}`).join(',')}}`;
  }
  return JSON.stringify(value);
}

async function sculptRenderContractSha256(
  contract: SculptRenderPipelineContract,
): Promise<string> {
  const bytes = new TextEncoder().encode(sculptStableJson(contract));
...
function sculptSceneRenderReceipt(scene: THREE.Scene): SculptRenderReceipt | undefined {
  return sculptRenderPipelineRegistry().get(scene.uuid)?.receipt();
}

export function recommendedSculptRendererOptions(
  contract: SculptRenderPipelineContract = SCULPT_REVIEW_RENDER_CONTRACT,
): { antialias: boolean } {
  const mode = contract.antiAliasing.mode;
  return { antialias: mode === 'auto' || mode === 'native-msaa' };
}

export async function createSculptReviewPipeline({
  renderer,
  scene,
  camera,
  contract = SCULPT_REVIEW_RENDER_CONTRACT,
}: {
  renderer: THREE.WebGLRenderer;
  scene: THREE.Scene;
  camera: THREE.Camera;
  contract?: SculptRenderPipelineContract;
}): Promise<SculptReviewPipelineController> {
  const registry = sculptRenderPipelineRegistry();
  const reservations = sculptRenderPipelineReservations();
  const existing = registry.get(scene.uuid);
  if ((existing && !existing.receipt().disposed) || reservations.has(scene.uuid)) {
    throw new Error('scene already has an active sculpt review pipeline');
  }
  if (contract.version !== 1 || contract.backend !== 'webgl') {
    throw new Error('unsupported sculpt render pipeline contract');
  }

  const contextAttributes = renderer.getContextAttributes();
  const gl = renderer.getContext();
  const contextAntialias = contextAttributes?.antialias === true;
  const defaultFramebufferSamples = Number(gl.getParameter(gl.SAMPLES) ?? 0);
  const nativeVerified = contextAntialias && defaultFramebufferSamples > 0;
  const requestedMode = contract.antiAliasing.mode;
  let resolvedMode: SculptResolvedRenderMode;
  let fallbackReason = '';

  if (requestedMode === 'off') {
    if (contract.status === 'required') {
      throw new Error('required sculpt render pipeline cannot disable anti-aliasing');
    }
    resolvedMode = 'off';
  } else if (requestedMode === 'native-msaa') {
    if (!nativeVerified) {
      throw new Error('native-msaa was requested but the existing WebGL context has no verified samples');
    }
    resolvedMode = 'native-msaa';
  } else if (requestedMode === 'auto' && nativeVerified) {
    resolvedMode = 'native-msaa';
  } else if (requestedMode === 'auto') {
    resolvedMode = contract.antiAliasing.fallbackOrder[0]
      ?? (contract.antiAliasing.qualityPreset === 'quality' ? 'smaa' : 'fxaa');
    fallbackReason = 'existing WebGL canvas has no verified native MSAA';
  } else {
    resolvedMode = requestedMode;
  }

  reservations.add(scene.uuid);
  let effectiveContractSha256: string;
  try {
    effectiveContractSha256 =
      contract === SCULPT_REVIEW_RENDER_CONTRACT
        ? SCULPT_REVIEW_RENDER_CONTRACT_SHA256
        : await sculptRenderContractSha256(contract);
  } catch (error: unknown) {
    reservations.delete(scene.uuid);
    throw error;
  }

  let composer: import('three/addons/postprocessing/EffectComposer.js').EffectComposer | null = null;
  const ownedPasses: Array<{ dispose?: () => void }> = [];
  let passChain = ['renderer'];
  let outputTransformOwner: SculptRenderReceipt['outputTransformOwner'] = 'renderer';
  if (resolvedMode === 'smaa' || resolvedMode === 'fxaa') {
    try {
      const [
        { EffectComposer },
        { RenderPass },
      ...
      ownedPasses.push(renderPass, outputPass);
      composer.addPass(renderPass);
      if (resolvedMode === 'smaa') {
        if (!('SMAAPass' in antiAliasModule)) {
          throw new Error('installed Three.js addons do not expose SMAAPass');
        }
        const smaaPass = new antiAliasModule.SMAAPass();
        ownedPasses.push(smaaPass);
        composer.addPass(smaaPass);
        composer.addPass(outputPass);
        passChain = ['RenderPass', 'SMAAPass', 'OutputPass'];
      } else {
        if (!('FXAAPass' in antiAliasModule)) {
          throw new Error('installed Three.js addons do not expose FXAAPass');
        }
        const fxaaPass = new antiAliasModule.FXAAPass();
        ownedPasses.push(fxaaPass);
        composer.addPass(outputPass);
        composer.addPass(fxaaPass);
        passChain = ['RenderPass', 'OutputPass', 'FXAAPass'];
      }
  ...
  let frameCount = 0;
  let disposed = false;
  const receipt = (): SculptRenderReceipt => ({
    artifactType: 'threejs-sculpt-render-receipt',
    version: 1,
    contractSha256: effectiveContractSha256,
    backend: 'webgl',
    requestedMode,
    resolvedMode,
    fallbackReason,
    passChain: [...passChain],
    outputTransformOwner,
    contextAntialias,
    defaultFramebufferSamples,
    maximumRenderTargetSamples: renderer.capabilities.maxSamples,
    antialiasVerified: resolvedMode !== 'off',
    logicalWidth,
    logicalHeight,
    pixelRatio,
    frameCount,
  ...
  const controller: SculptReviewPipelineController = {
    render: (deltaTime?: number): void => {
      if (disposed) throw new Error('cannot render with a disposed sculpt review pipeline');
      if (composer) composer.render(deltaTime);
      else renderer.render(scene, camera);
      frameCount += 1;
    },
    resize: (width: number, height: number, requestedPixelRatio: number): void => {
      if (disposed) throw new Error('cannot resize a disposed sculpt review pipeline');
      if (!(width > 0) || !(height > 0) || !(requestedPixelRatio > 0)) {
        throw new Error('render pipeline size and pixel ratio must be positive');
      }
      logicalWidth = width;
      logicalHeight = height;
      pixelRatio = Math.max(1, Math.min(contract.maxPixelRatio, requestedPixelRatio));
    ...
    dispose: (): void => {
      ...
      composer?.dispose();
      disposed = true;
      if (registry.get(scene.uuid) === controller) registry.delete(scene.uuid);
    },
  };
  reservations.delete(scene.uuid);
  registry.set(scene.uuid, controller);
  return controller;
}
"""
    return (
        ...
        .replace("__HASH__", json.dumps(contract_sha256))
        .strip()
        .splitlines()
    )
...
def generate(
    ...
    *,
    ...
    spec_sha256 = generation_validation_hash(spec, pass_id)
    review_render_contract = render_pipeline_contract(spec)
    review_render_contract_sha256 = render_pipeline_contract_sha256(spec)
    spec = generation_spec_projection(spec, pass_id)
    shading_treatment = assessed_shading_treatment(spec)
    ...
    emit_local_details = pass_id not in {"blockout", "structure", "structural-pass"}
    ...
    lines: list[str] = [
        ...
        "  materialStatus: 'pending' | 'ready' | 'error';",
        *(
            ["  renderPipeline?: SculptRenderReceipt;"]
            if review_render_contract is not None
            else []
        ),
        "};",
        "",
        ...
        *(
            [
                *typescript_review_pipeline_helpers(
                    review_render_contract,
                    review_render_contract_sha256,
                ),
                "",
            ]
            if review_render_contract is not None
        ...
        "    materialStatus: runtime.materialStatus(),",
        *(
            [
                "    renderPipeline: scene ? sculptSceneRenderReceipt(scene) : undefined,"
            ]
            if review_render_contract is not None
            else []
        ),
        ...
        f"const SCULPT_SHADING_TREATMENT: string = {json.dumps(shading_treatment)};",
        ...
        "    const oxidized = type === 'rust' || type === 'oxide' || type === 'patina';",
        "    const defaultColor = wet ? '#1B2024'",
        "      : worn ? '#B8AE9B'",
        "      : type === 'rust' ? '#8C3F1F'",
        "      : type === 'oxide' ? '#594B3B'",
        "      : type === 'patina' ? '#47756B'",
        ...
        "      amount,",
        "      color,",
        "      roughnessDelta: Math.max(-1, Math.min(1, readLayerNumber(source.roughnessDelta, ['base', 'amount'], wet || worn ? -0.22 : oxidized ? 0.36 : 0.18))),",
        "      metalnessDelta: Math.max(-1, Math.min(1, readLayerNumber(source.metalnessDelta, ['base', 'amount'], oxidized ? -0.92 : 0))),",
        "      heightDelta: Math.max(-0.25, Math.min(0.25, readLayerNumber(source.heightDelta, ['base', 'amount'], worn ? -0.025 : oxidized ? 0.014 : 0.006))),",
        ...
        "}",
        "",
        "export function applySculptMaterialProjectionToGeometry(",
        "  geometry: THREE.BufferGeometry,",
        "  spec: SculptMaterialSpec,",
        "  effectiveScale = new THREE.Vector3(1, 1, 1),",
        "): void {",
        "  const projection = spec.textureProjection;",
        "  if (!projection || typeof projection !== 'object') return;",
        "  const mode = typeof projection.mode === 'string' ? projection.mode : 'uv';",
        "  if (mode === 'uv') return;",
        "  if (mode !== 'planar' && mode !== 'cylindrical' && mode !== 'spherical') return;",
        "  const positions = geometry.getAttribute('position');",
        "  if (!positions || positions.itemSize < 3) return;",
        "  geometry.computeBoundingBox();",
        "  const bounds = geometry.boundingBox;",
        "  if (!bounds || bounds.isEmpty()) return;",
        "  const size = bounds.getSize(new THREE.Vector3()).multiply(new THREE.Vector3(",
        "    Math.abs(effectiveScale.x), Math.abs(effectiveScale.y), Math.abs(effectiveScale.z),",
        "  ));",
        "  const center = bounds.getCenter(new THREE.Vector3());",
        "  const safe = new THREE.Vector3(Math.max(size.x, 1e-6), Math.max(size.y, 1e-6), Math.max(size.z, 1e-6));",
        "  const requestedAxis = projection.axis === 'x' ? 0 : projection.axis === 'y' ? 1 : projection.axis === 'z' ? 2 : null;",
        "  const majorAxis = requestedAxis ?? (safe.x >= safe.y && safe.x >= safe.z ? 0 : safe.y >= safe.z ? 1 : 2);",
        "  const planarDropAxis = requestedAxis ?? (safe.x <= safe.y && safe.x <= safe.z ? 0 : safe.y <= safe.z ? 1 : 2);",
        "  const uvs = new Float32Array(positions.count * 2);",
        "  const point = new THREE.Vector3();",
        "  const direction = new THREE.Vector3();",
        "  for (let index = 0; index < positions.count; index += 1) {",
        "    point.fromBufferAttribute(positions, index).sub(center).multiply(effectiveScale);",
        "    let u = 0;",
        "    let v = 0;",
        "    if (mode === 'spherical') {",
        "      direction.copy(point).normalize();",
        "      u = 0.5 + Math.atan2(direction.z, direction.x) / (Math.PI * 2);",
        "      v = 0.5 - Math.asin(THREE.MathUtils.clamp(direction.y, -1, 1)) / Math.PI;",
        "    } else if (mode === 'cylindrical') {",
        "      if (majorAxis === 0) {",
        "        u = 0.5 + Math.atan2(point.z, point.y) / (Math.PI * 2);",
        "        v = point.x / safe.x + 0.5;",
        "      } else if (majorAxis === 2) {",
        "        u = 0.5 + Math.atan2(point.y, point.x) / (Math.PI * 2);",
        "        v = point.z / safe.z + 0.5;",
        "      } else {",
        "        u = 0.5 + Math.atan2(point.z, point.x) / (Math.PI * 2);",
        "        v = point.y / safe.y + 0.5;",
        "      }",
        "    } else {",
        "      if (planarDropAxis === 0) { u = point.z / safe.z + 0.5; v = point.y / safe.y + 0.5; }",
        "      else if (planarDropAxis === 1) { u = point.x / safe.x + 0.5; v = point.z / safe.z + 0.5; }",
        "      else { u = point.x / safe.x + 0.5; v = point.y / safe.y + 0.5; }",
        "    }",
        "    uvs[index * 2] = u;",
        "    uvs[index * 2 + 1] = v;",
        "  }",
        "  geometry.setAttribute('uv', new THREE.BufferAttribute(uvs, 2));",
        "  geometry.userData.sculptTextureProjection = mode;",
        "}",
        "",
        ...
        "  const assessments = reference.channelAssessments;",
        "  const assessment = assessments && typeof assessments === 'object'",
        "    ? (assessments as Record<string, unknown>)[channel] : null;",
        "  if (assessment && typeof assessment === 'object' && (assessment as Record<string, unknown>).eligible === false) return null;",
        "  return typeof record.url === 'string' && record.url.trim() ? record.url : null;",
        "}",
        "",
        "function authoredMapUrl(spec: SculptMaterialSpec, channel: string): string | null {",
        "  const set = spec.textureSet;",
        "  if (!set || typeof set !== 'object' || set.status !== 'ready') return null;",
        "  if (!['reference-extracted', 'imagegen-authored', 'external-authored'].includes(String(set.sourceType))) return null;",
        "  const channels = set.channels;",
        "  if (!channels || typeof channels !== 'object') return null;",
        "  const map = (channels as Record<string, unknown>)[channel];",
        "  if (!map || typeof map !== 'object') return null;",
        "  const record = map as Record<string, unknown>;",
        "  if (record.eligible === false) return null;",
        ...
        "    url,",
        "    () => { texture.userData.sculptLoadStatus = 'ready'; resolveReady(); },",
        "    undefined,",
        "    () => {",
        "      texture.userData.sculptLoadStatus = 'error';",
        "      rejectReady(new Error(`failed to load material texture: ${url}`));",
        "    },",
        "  );",
        "  texture.userData.sculptLoadStatus = 'pending';",
        "  texture.userData.sculptReady = ready;",
        "  const projection = spec.textureProjection && typeof spec.textureProjection === 'object' ? spec.textureProjection : {};",
        ...
        "}",
        "",
        "function styleUsesTextureChannel(channel: string): boolean {",
        "  if (SCULPT_SHADING_TREATMENT === 'unlit') return channel === 'albedo';",
        "  if (SCULPT_SHADING_TREATMENT === 'toon-ramp' || SCULPT_SHADING_TREATMENT === 'cel-banded')",
        "    return channel === 'albedo' || channel === 'normal';",
        "  if (SCULPT_SHADING_TREATMENT === 'smooth-lit') return channel !== 'height' && channel !== 'normal';",
        "  return true;",
        "}",
        "",
        ...
        "    roughness: roughness && styleUsesTextureChannel('roughness') ? createLoadedMapTexture(roughness, THREE.NoColorSpace, spec, options) : undefined,",
        "    metalness: metalness && styleUsesTextureChannel('metalness') ? createLoadedMapTexture(metalness, THREE.NoColorSpace, spec, options) : undefined,",
        "    height: height && styleUsesTextureChannel('height') ? createLoadedMapTexture(height, THREE.NoColorSpace, spec, options) : undefined,",
        "    normal: normal && styleUsesTextureChannel('normal') ? createLoadedMapTexture(normal, THREE.NoColorSpace, spec, options) : undefined,",
        "    ao: ao && styleUsesTextureChannel('ao') ? createLoadedMapTexture(ao, THREE.NoColorSpace, spec, options) : undefined,",
        "    source: 'reference-pixel-extraction',",
        "  };",
        "}",
        "",
        "function makeAuthoredTextureSet(spec: SculptMaterialSpec, options: ProceduralModelOptions): ProceduralTextureSet | null {",
        "  const albedo = authoredMapUrl(spec, 'albedo');",
        "  if (!albedo) return null;",
        "  const load = (channel: string): THREE.Texture | undefined => {",
        "    if (!styleUsesTextureChannel(channel)) return undefined;",
        "    const url = authoredMapUrl(spec, channel);",
        "    return url ? createLoadedMapTexture(url, channel === 'albedo' ? THREE.SRGBColorSpace : THREE.NoColorSpace, spec, options) : undefined;",
        "  };",
        "  return {",
        "    albedo: load('albedo')!,",
        "    roughness: load('roughness'), metalness: load('metalness'), height: load('height'),",
        "    normal: load('normal'), ao: load('ao'),",
        "    source: String(spec.textureSet?.sourceType ?? 'authored'),",
        "  };",
        "}",
        "",
        ...
        "}",
        "",
        "function routeSculptMaterialStyle(",
        "  material: THREE.MeshPhysicalMaterial, textures: ProceduralTextureSet | null, options: ProceduralModelOptions,",
        "): THREE.Material {",
        "  material.userData.shadingTreatment = SCULPT_SHADING_TREATMENT;",
        "  if (SCULPT_SHADING_TREATMENT === 'flat-lit') { material.flatShading = true; return material; }",
        "  if (SCULPT_SHADING_TREATMENT === 'emissive-dominant') {",
        "    material.emissive.copy(material.color); material.emissiveMap = textures?.albedo ?? null;",
        "    material.emissiveIntensity = Math.max(1, material.emissiveIntensity); return material;",
        "  }",
        "  if (SCULPT_SHADING_TREATMENT !== 'unlit' && SCULPT_SHADING_TREATMENT !== 'toon-ramp'",
        "      && SCULPT_SHADING_TREATMENT !== 'cel-banded') return material;",
        "  const common = {",
        "    color: material.color, map: textures?.albedo ?? null, opacity: material.opacity,",
        "    transparent: material.transparent, alphaTest: material.alphaTest, wireframe: options.wireframe ?? false,",
        "    side: material.side,",
        "  };",
        "  const routed: THREE.Material = SCULPT_SHADING_TREATMENT === 'unlit'",
        "    ? new THREE.MeshBasicMaterial(common)",
        "    : new THREE.MeshToonMaterial({ ...common, normalMap: textures?.normal ?? null });",
        "  routed.userData = { ...material.userData, shadingTreatment: SCULPT_SHADING_TREATMENT };",
        "  material.dispose();",
        "  return routed;",
        "}",
        "",
        ...
        "  const textures = makeAuthoredTextureSet(spec, options) ?? makeReferenceTextureSet(spec, options) ?? makeProceduralTextureSet(id, spec, options, profile);",
        "  const material = new THREE.MeshPhysicalMaterial({",
        "    color: textures ? 0xffffff : new THREE.Color(typeof spec.baseColor === 'string' ? spec.baseColor : '#8A7A5F'),",
        "    roughness: textures?.roughness ? 1 : clamp01(readLayerNumber(spec.roughness, ['base'], 0.76)),",
        ...
        "  }",
        "  if (profile === 'fiber' || spec.anisotropy !== undefined) {",
        "    material.anisotropy = clamp01(readLayerNumber(spec.anisotropy, ['base', 'amount'], profile === 'fiber' ? 0.72 : 0));",
        ...
        "    if (textures.normal && SCULPT_SHADING_TREATMENT !== 'smooth-lit') {",
        "      material.normalMap = textures.normal;",
        "      material.normalScale.setScalar(Math.max(0, readLayerNumber(spec.normal, ['strength', 'amplitude'], 0.35)));",
        "    }",
        "    if (textures.ao) {",
        "      material.aoMap = textures.ao; material.aoMap.channel = 0;",
        "      material.aoMapIntensity = readLayerNumber(spec.ambientOcclusion, ['cavityStrength', 'strength'], 0.35);",
        "    }",
        "    const bumpScale = Math.max(0, readLayerNumber(spec.bump, ['amplitude', 'strength'], 0));",
        "    if (bumpScale > 0 && textures.height && SCULPT_SHADING_TREATMENT !== 'smooth-lit') {",
        "      material.bumpMap = textures.height;",
        "      material.bumpScale = bumpScale;",
        "    }",
        "    const displacementScale = Math.max(0, readLayerNumber(spec.displacement, ['amplitude', 'strength'], 0));",
        "    if (displacementScale > 0 && textures.height && SCULPT_SHADING_TREATMENT !== 'smooth-lit') {",
        ...
        "  return routeSculptMaterialStyle(material, textures, options);",
        "}",
        "",
        "function sculptMaterialReady(materialMap: Record<string, THREE.Material>): Promise<void> {",
        "  const waits = new Set<Promise<void>>();",
        "  for (const material of Object.values(materialMap)) {",
        "    const values = [...Object.values(material as unknown as Record<string, unknown>), material.userData.heightMap];",
        "    for (const value of values) {",
        "      if (!(value instanceof THREE.Texture)) continue;",
        "      const ready = value.userData.sculptReady;",
        "      if (ready && typeof (ready as Promise<void>).then === 'function') waits.add(ready as Promise<void>);",
        "    }",
        "  }",
        "  return Promise.all([...waits]).then(() => undefined);",
        "}",
        "",
        "function sculptMaterialStatus(materialMap: Record<string, THREE.Material>): 'pending' | 'ready' | 'error' {",
        "  let status: 'pending' | 'ready' | 'error' = 'ready';",
        "  for (const material of Object.values(materialMap)) {",
        "    const values = [...Object.values(material as unknown as Record<string, unknown>), material.userData.heightMap];",
        "    for (const value of values) {",
        "      if (!(value instanceof THREE.Texture)) continue;",
        "      if (value.userData.sculptLoadStatus === 'error') return 'error';",
        "      if (value.userData.sculptLoadStatus === 'pending') status = 'pending';",
        "    }",
        "  }",
        "  return status;",
        "}",
        ...
        *typescript_geometry_helpers(geometry_helpers),
        "// Generator contract: ObjectSculptSpec 3.2 + hash-bound visual evidence v1.",
        ...
        "  root.userData.generatorContract = 'object-sculpt-3.2/evidence-v1';",
        "",
    ...
    lines.extend(
        [
            "",
            "  const materialReady = sculptMaterialReady(materialMap);",
            "  const materialStatus = (): 'pending' | 'ready' | 'error' => sculptMaterialStatus(materialMap);",
    ...
    for index, component in enumerate(components):
        ...
        if emission is not None:
            ...
            lines.append(
                f"  applySculptMaterialProjectionToGeometry("
                f"{component_var}Geometry, {json_literal(materials.get(material_id, {}))}, {node_var}.scale);"
            )
            material_expression = (
        ...
        if emit_local_details and emission is not None:
            ...
            if isinstance(local_features, list):
                for feature_index, feature in enumerate(local_features):
                    ...
                    lines.extend(
                        [
                            f"  const {feature_var}Geometry = {feature_geometry};",
                            f"  applySculptMaterialProjectionToGeometry("
                            f"{feature_var}Geometry, {json_literal(materials.get(feature_material_id, materials.get(material_id, {})))}, "
                            f"new THREE.Vector3({vector(feature_scale, [1, 1, 1])}));",
                            f"  const {feature_var} = new THREE.Mesh(",
                            f"    {feature_var}Geometry,",
    ...
    lines.extend(
        [
            ...
            "  };",
            "  root.userData.sculptRuntime = { nodes, meshes, instances, sockets, colliders, destructionGroups, materialReady, materialStatus, dispose } satisfies ProceduralModelRuntime;",
            ...
            "    note: 'Await root.userData.sculptRuntime.materialReady before Lookdev review; call dispose when removing the model.',",
            "  };",
            ...
            "}",
            "",
            "export function installSculptLookDevEnvironmentTarget(",
            "  scene: THREE.Scene,",
            ...
            "  if (useAsBackground) scene.background = target.texture;",
            "  return () => {",
            "    if (scene.environment === target.texture) scene.environment = previousEnvironment;",
            "    if (useAsBackground && scene.background === target.texture) scene.background = previousBackground;",
            "    target.dispose();",
            "    pmrem.dispose();",
            "  };",
            "}",
            "",
            f"export function configure{type_name}LookDevEnvironment(",
            "  scene: THREE.Scene,",
            "  renderer: THREE.WebGLRenderer,",
            ...
            "    studio.background = new THREE.Color(0x121820);",
            "    const resources: Array<THREE.Mesh<THREE.PlaneGeometry, THREE.MeshBasicMaterial>> = [];",
            "    const panel = (color: number, intensity: number, size: [number, number], position: [number, number, number]): void => {",
            ...
            "      mesh.lookAt(0, 0, 0);",
            "      mesh.userData.sculptValidationRole = 'environment';",
            "      studio.add(mesh);",
            "      resources.push(mesh);",
            "    };",
            "    panel(0xffead0, 1.8, [5.5, 7.0], [-4.5, 4.0, 3.0]);",
            "    panel(0xa9c9ff, 0.75, [4.0, 5.0], [4.0, 2.0, 2.5]);",
            "    panel(0xffffff, 1.15, [3.0, 4.5], [0.0, 5.5, -3.5]);",
            "    panel(0x27313c, 0.55, [8.0, 8.0], [0.0, -4.0, 0.0]);",
            "    target = pmrem.fromScene(studio, 0.04, 0.1, 100);",
            "    for (const mesh of resources) { mesh.geometry.dispose(); mesh.material.dispose(); }",
            "  }",
            "  return installSculptLookDevEnvironmentTarget(scene, target, pmrem, useAsBackground);",
            "}",
            "",
...
def write_generated_spec(
    ...
    *,
    ...
    _module_preview: bool = False,
) -> dict[str, Any]:
    """Validate and write one generated factory; shared by legacy and fast-path CLIs."""

    selected_pass = pass_id or unlocked_pass(spec)
    if _module_preview and _validation_proof is None:
        raise ValueError(
            "private module preview generation requires a hash-bound module validation proof"
        )
    ...
    if (
        selected_pass != "blockout"
        and has_geometry_parts
    ...
    if resolved_output.exists() and force:
        ...
        # Accept legacy factories whose marker was emitted after the helper block,
        # while new factories put the exact marker on the first line.
        if GENERATED_MARKER not in existing.splitlines():
            raise ValueError(

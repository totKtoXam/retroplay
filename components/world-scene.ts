...
export function createWorldScene(renderer?: T.WebGLRenderer) {
  ...
  // Центральный двухэтажный корпус: открытый вход, окна, лестница, 2 этаж и шанырак.
  const yurt = new T.Group();
  decor.add(yurt);

  // 1-й этаж: пол
  box(9.2, 0.2, 9.2, '#d2c7bf', 0, 0.1, -18, yurt);

  // Южный фасад (Вход z = -13.5)
  // Стена слева (x in [-4.6, -1.4])
  box(3.2, 3.5, 0.3, '#e8e2df', -3.0, 1.75, -13.5, yurt);
  // Стена справа (x in [1.4, 4.6])
  box(3.2, 3.5, 0.3, '#e8e2df', 3.0, 1.75, -13.5, yurt);
  // Притолока над дверью (проём свободен: x in [-1.4, 1.4], y in [0.2, 2.6])
  box(2.8, 0.9, 0.3, '#324664', 0, 3.05, -13.5, yurt);
  // Козырек над крыльцом
  box(4.4, 0.14, 1.8, '#586c8b', 0, 2.7, -12.7, yurt);
  cyl(0.08, 0.08, 2.6, '#b39782', -1.9, 1.3, -12.0, yurt, 8);
  cyl(0.08, 0.08, 2.6, '#b39782', 1.9, 1.3, -12.0, yurt, 8);
  // Входной коврик
  box(3.2, 0.06, 1.6, '#8a78a2', 0, 0.03, -13.0, yurt);
  // Декоративный орнамент фасада
  for (let i = 0; i < 6; i++) {
    box(
      0.28,
      0.22,
  ...
  box(1.4, 3.5, 0.3, '#e8e2df', -3.9, 1.75, -22.5, yurt);
  box(2.0, 3.5, 0.3, '#e8e2df', 0, 1.75, -22.5, yurt);
  box(1.4, 3.5, 0.3, '#e8e2df', 3.9, 1.75, -22.5, yurt);
  // Окна 1 этажа (подоконники y: 0..0.95, перемычки y: 2.5..3.5, проёмы открыты для прыжка!)
  box(2.2, 0.95, 0.3, '#b6a99e', -2.1, 0.475, -22.5, yurt);
  box(2.2, 1.0, 0.3, '#586c8b', -2.1, 3.0, -22.5, yurt);
  box(2.2, 0.95, 0.3, '#b6a99e', 2.1, 0.475, -22.5, yurt);
  box(2.2, 1.0, 0.3, '#586c8b', 2.1, 3.0, -22.5, yurt);

  // Западный фасад (x = -4.5)
  box(0.3, 3.5, 2.2, '#e8e2df', -4.5, 1.75, -14.4, yurt);
  box(0.3, 3.5, 1.0, '#e8e2df', -4.5, 1.75, -18.0, yurt);
  box(0.3, 3.5, 2.2, '#e8e2df', -4.5, 1.75, -21.6, yurt);
  box(0.3, 0.95, 2.0, '#b6a99e', -4.5, 0.475, -16.5, yurt);
  box(0.3, 1.0, 2.0, '#586c8b', -4.5, 3.0, -16.5, yurt);
  box(0.3, 0.95, 2.0, '#b6a99e', -4.5, 0.475, -19.5, yurt);
  box(0.3, 1.0, 2.0, '#586c8b', -4.5, 3.0, -19.5, yurt);
  ...
  for (let s = 0; s < 7; s++) {
    const stepZ = -15.6 - s * 0.8;
    const stepY = 0.3 + s * 0.5;
    box(1.5, 0.5, 0.78, '#b39782', -3.4, stepY, stepZ, yurt);
  }
  // Перила лестницы
  box(0.08, 0.9, 5.6, '#56667e', -2.6, 2.4, -18.0, yurt);

  // Перекрытие 2-го этажа (y = 3.6, с проёмом для выхода с лестницы)
  box(6.8, 0.2, 8.8, '#dee2e7', 1.0, 3.5, -18.0, yurt);
  box(2.0, 0.2, 2.8, '#dee2e7', -3.4, 3.5, -21.0, yurt);
  box(2.0, 0.2, 1.6, '#dee2e7', -3.4, 3.5, -14.4, yurt);
  // Ограждение лестничного проёма на 2-м этаже
  box(0.08, 0.85, 4.4, '#56667e', -2.5, 4.025, -17.4, yurt);
  box(1.8, 0.85, 0.08, '#56667e', -3.4, 4.025, -15.2, yurt);

  // 2-й этаж: панорамный балкон на юг (z = -13.5, парапет y: 3.5..4.35, перемычка y: 6.2..7.0)
  box(9.2, 0.85, 0.3, '#324664', 0, 3.925, -13.5, yurt);
  box(9.2, 0.8, 0.3, '#e8e2df', 0, 6.6, -13.5, yurt);
  box(0.4, 1.85, 0.3, '#56667e', -4.3, 5.275, -13.5, yurt);
  box(0.4, 1.85, 0.3, '#56667e', 4.3, 5.275, -13.5, yurt);

  // 2-й этаж: стены и окна
  box(9.2, 0.9, 0.3, '#e8e2df', 0, 3.95, -22.5, yurt);
  box(9.2, 0.8, 0.3, '#e8e2df', 0, 6.6, -22.5, yurt);
  box(0.3, 0.9, 8.8, '#e8e2df', -4.5, 3.95, -18.0, yurt);
  box(0.3, 0.8, 8.8, '#e8e2df', -4.5, 6.6, -18.0, yurt);
  box(0.3, 0.9, 8.8, '#e8e2df', 4.5, 3.95, -18.0, yurt);
  box(0.3, 0.8, 8.8, '#e8e2df', 4.5, 6.6, -18.0, yurt);

  // Крыша (y = 6.8..7.1)
  box(9.8, 0.3, 9.8, '#d4c1ab', 0, 6.95, -18.0, yurt);

  // Шанырак на крыше
  const crown = mesh(
    new T.TorusGeometry(1.8, 0.12, 6, 28),
    '#ddbe77',
    0,
    7.2,
    -18,
  ...
  for (let i = -2; i <= 2; i++) {
    const len = Math.sqrt(Math.max(0.2, 3.24 - i * i * 0.45)) * 2;
    box(len, 0.05, 0.07, '#d4c1ab', 0, 7.2, -18 + i * 0.55, yurt);
  }

  // Мебель внутри корпуса
  cyl(1.4, 1.4, 0.45, '#bd9b7c', 0.8, 0.425, -18, yurt, 16);
  box(1.8, 0.35, 0.6, '#669feb', 0.8, 0.375, -16.8, yurt);
  box(1.8, 0.35, 0.6, '#e49b9e', 0.8, 0.375, -19.2, yurt);
  box(4.0, 0.35, 0.6, '#aa8f83', 0, 3.775, -14.4, yurt);
  ...
  const colliders: BoxCollider3D[] = ALL_3D_COLLIDERS;

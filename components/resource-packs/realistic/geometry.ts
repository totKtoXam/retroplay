...
export function geometryBatch(parent: T.Object3D) {
  ...
  function add(geometry: T.BufferGeometry, material: T.Material, position: number[], rotation: number[] = [0, 0, 0]) {
    const m = new T.Matrix4().compose(new T.Vector3(...position),
      new T.Quaternion().setFromEuler(new T.Euler(...rotation)), new T.Vector3(1, 1, 1));
    const geo = geometry.index ? geometry.toNonIndexed() : geometry.clone();
    geometry.dispose(); geo.applyMatrix4(m);
  ...
  return {
    ...
    box(size: number[], mat: T.Material, pos: number[], radius = 0.018) {
      add(new RoundedBoxGeometry(size[0], size[1], size[2], 1, Math.min(radius, ...size.map((v) => v / 4))), mat, pos);
    },
    pipe(a: number[], b: number[], radius: number, mat: T.Material) {
      ...
      geo.applyQuaternion(new T.Quaternion().setFromUnitVectors(new T.Vector3(0, 1, 0), dir.normalize()));
    ...
    finish() {
      ...
      buckets.forEach((geos, mat) => {
        ...
        visualOnly(mesh); mesh.castShadow = true; mesh.receiveShadow = true;
        parent.add(mesh); meshes.push(mesh);
      });
      buckets.clear(); return meshes;
    },
  };
}
...
export function garment(rings: number[][], radial = 20) {
  ...
  for (const row of [0, rings.length - 1]) for (let col = 1; col < radial - 1; col++) {

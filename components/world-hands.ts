...
export function createFirstPersonHands(camera: T.Camera) {
  ...
  return {
    group,
    /**
     * Стекло коллиматора. По нему в `world.tsx` считается прямоугольник окна
     * на экране: подсветка целей должна жить внутри рамки прицела, а где эта
     * рамка — знает только сама модель.
     */
    opticGlass: paintGlass,
    /**

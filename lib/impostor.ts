...
export const TASK_MS: Record<TaskKind, number> = {
  ...
  code: 3000,
  upload: 5000,
  // Астероиды нужно сбивать по одному — быстрее набора попаданий не пройти.
  asteroids: 5000,
  // Один честный взмах картой уже укладывается в это время; повторные — из-за промаха со скоростью.
  swipe: 2200,
  leaves: 4000,
  // Шесть шестиугольников кликом каждый — быстрее их не погасить.
  shields: 2500,
  // Нужно ещё и удержать метку на линии, не только найти её.
  align: 2600,
  // Показ и повтор растущей последовательности сам по себе занимает время.
  simon: 7000,
  fuel: 4000,
};
const INTRO_MS = 5000;

...
} from '../components/resource-packs/realistic/materials.ts';
...
test('Saved pack IDs validate independently of room state; existing quality presets bound GPU allocation', () => {
  ...
  assert.equal(
    visualBudget('high').textureSize,
    visualBudget('cinematic').textureSize,
  );
  assert.equal(visualBudget('low').localLights, 0);
  assert.ok(
    visualBudget('low').detailDistance <
      visualBudget('cinematic').detailDistance,
  );
});
test('Original PBR maps are deterministic, finite and contain real normal/roughness variation', () => {
  for (const surface of [
    ...
    const a = surfacePixels(surface, 64),
      b = surfacePixels(surface, 64);
...
function rayHits(root, ray) {
  ...
  root.traverse((o) => {
    if (o instanceof T.Mesh && visibleInWorld(o)) meshes.push(o);
  });
  return ray
    .intersectObjects(meshes, false)
    .map((h) => [h.object.id, ...h.point.toArray().map((v) => +v.toFixed(7))]);
}
  ...
  for (const anime of [false, true]) {
    ...
    avatar.traverse((o) => {
      if (o instanceof T.Mesh)
        canonicalMeshes.push([o, o.geometry, o.material, o.visible]);
    });
    const rays = [];
    for (let y = 0.15; y < 2.5; y += 0.13)
      for (const x of [-0.22, 0, 0.22])
        rays.push(
          new T.Raycaster(new T.Vector3(x, y, -5), new T.Vector3(0, 0, 1)),
        );
    const before = rays.map((r) => rayHits(avatar, r));
    const skin = dressFieldCharacter(avatar, library);
    assert.deepEqual(
      rays.map((r) => rayHits(avatar, r)),
      before,
      'all collision points and original mesh identities match',
    );
    const twin = createAvatar('#8196dd');
    setAvatarStyle(twin, anime);
    const names = [
      'rig',
      'head',
      'chest',
    ...
    for (const stance of ['stand', 'sit', 'lie'])
      for (const tool of ['paint', 'confetti', 'grenade', 'sniper', 'tablet']) {
        const motion = {
          speed: 3,
          strafe: 0.5,
          forward: 1,
          airborne: stance === 'stand',
          velocityY: 3,
          stance,
          tool,
          pitch: 0.2,
        ...
        for (let i = 0; i < 20; i++) {
          animateAvatar(avatar, motion, 1 / 30, i / 30);
          animateAvatar(twin, motion, 1 / 30, i / 30);
        }
        avatar.updateMatrixWorld(true);
        twin.updateMatrixWorld(true);
        for (const name of names)
          assert.deepEqual(
            avatar.getObjectByName(name).matrix.elements,
            twin.getObjectByName(name).matrix.elements,
            name,
          );
    ...
    assert.equal(
      visibleInWorld(
        avatar
          .getObjectByName('unmasked-head')
          .getObjectByName('field-uniform'),
      ),
      false,
    );
    assert.equal(avatar.getObjectByName('anonymous-bag').visible, true);
    skin.dispose();
    for (const [mesh, geometry, material, visible] of canonicalMeshes) {
...
test('Decorative geometry cannot change camera obstacles or shooting; instancing and LOD bound cost', () => {
  ...
  const wall = new T.Mesh(
    new T.BoxGeometry(10, 10, 0.5),
    new T.MeshStandardMaterial(),
  );
  wall.position.z = -6;
  scene.add(wall);
  const camera = new T.PerspectiveCamera();
  const eye = new T.Vector3(0, 2, 0),
    desired = new T.Vector3(0, 2, -10);
  scene.updateMatrixWorld(true);
  const before = avoidCameraWalls(eye, desired, [wall]);
  const pack = createFieldEnvironment(scene, library, 'low', [
    [-9, -6],
    [9, -6],
    [-9, 9],
    [9, 9],
  ...
  scene.updateMatrixWorld(true);
  assert.deepEqual(
    avoidCameraWalls(eye, desired, meshes).toArray(),
    before.toArray(),
  );
  const hit = new T.Raycaster(
    eye,
    desired.clone().sub(eye).normalize(),
  ).intersectObjects(meshes, false);
  assert.ok(hit.every((h) => h.object === wall));
  assert.ok(
    meshes.length < 20,
    'environment is batched, not one draw per prop',
  );
  let geometries = 0,
    disposed = 0;
